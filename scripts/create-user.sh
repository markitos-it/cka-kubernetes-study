...
# Convert comma-separated groups to /O=group1/O=group2/O=group3 format
GROUPS_FORMATTED=""
...
openssl req -new -key ${USERNAME}-key.pem -out ${USERNAME}.csr -subj "/CN=${USERNAME}${GROUPS_FORMATTED}"
...
# Get the certificate
kubectl get csr ${USERNAME} -o jsonpath='{.status.certificate}' | base64 -d > ${USERNAME}-cert.pem

# Collect cluster-side info
...
CERT_SUBJECT=$(openssl x509 -in ${USERNAME}-cert.pem -noout -subject | sed 's/^subject= //')
CERT_ISSUER=$(openssl x509 -in ${USERNAME}-cert.pem -noout -issuer | sed 's/^issuer= //')
CERT_DATES=$(openssl x509 -in ${USERNAME}-cert.pem -noout -dates | tr '\n' ' ')
CERT_SERIAL=$(openssl x509 -in ${USERNAME}-cert.pem -noout -serial)
...
echo "   Private key: ${USERNAME}-key.pem"
echo "   Certificate: ${USERNAME}-cert.pem"
...
echo "     kubectl config set-credentials ${USERNAME} --client-certificate=${USERNAME}-cert.pem --client-key=${USERNAME}-key.pem"
